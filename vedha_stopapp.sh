...
exit 1
else echo "${var_USER} attempting to stop Application - ${arg_APP} "
fi
...
if [ "${RC}" == "0" ];then
echo "${var_USER} STOPPED the Application ${arg_APP} - SUCCESS "
else
echo "${var_USER} STOPPED the Application ${arg_APP} - FAILED"
exit 1
fi
